...
\section{1. Why a Calendar}\label{why-a-calendar}}
...
\section{2. The Revelation as Phase-Space Coordinate}\label{the-revelation-as-phase-space-coordinate}}
...
\textbf{Stellar position:} Local Sidereal Time at the kuti was 13.82 hours (207.29°). This means the meridian --- the line running due south through the zenith --- passed through Right Ascension 13h49m. Arcturus (α Boötis), the brightest star in the northern celestial hemisphere and the fourth-brightest in the sky, was on or very near the meridian at its culmination. The star the classical Arabic astronomers called \emph{al-Simāk al-Rāmiḥ} --- the Elevated Lance-Bearer.
...
\section{3. The Three Incommensurabilities}\label{the-three-incommensurabilities}}
...
No two of these three cycles share a common measure. No ratio between them is rational. The system is irreducibly aperiodic --- it never repeats exactly. The closest approach to recurrence is the Metonic cycle (approximately 19 solar years ≈ 235 synodic months), after which the sun and moon return to approximately the same relative positions. The first Metonic near-return from the revelation anchor falls in approximately June--July 2044.
...
\section{4. The Lunation as Breath}\label{the-lunation-as-breath}}

The first cycle of the Taqwīm --- the twelve lunations from the revelation --- reveals a striking pattern in lunation lengths:
...
The pattern is sinusoidal. The lunations breathe --- expanding from Daʿwah to Shahādah (the active arc grows slower and longer), then contracting from ʿAwdah through Dhāt (the contemplative arc quickens and shortens). The longest lunation (Shahādah, 29.79 days) coincides with the station of Witness --- as if the cosmos dwells longer in the act of testimony. The shortest (Waṣl/Dhāt, 29.29--29.34 days) coincides with Connection and Self --- the emptiest stations, where the calendar itself accelerates toward silence.
...
\section{5. The Surplus and the Gap}\label{the-surplus-and-the-gap}}

At the revelation anchor, the earth's solar and sidereal rotational counts were synchronised by definition (both set to zero). As of 2 May 2026, 308.69 solar days have elapsed --- but 309.54 sidereal days. The surplus is 0.8452 extra rotations that the sun-clock doesn't register.

By the first solar return (28 June 2026), the surplus will reach approximately 1.0 --- one full invisible rotation. The earth will have turned once more than the sun-days count. This is not a rounding error. It is a real extra turn of the planet, accumulated in the gap between two ways of measuring the same motion. The surplus is the rotational analogue of the 11-day annual drift between lunar and solar years: a quantity that is real, measurable, and systematically ignored by every calendar that chooses one reference frame and discards the other.
...
\section{6. The Seasonal Migration}\label{the-seasonal-migration}}
...
\section{7. Assel's Question}\label{assels-question}}
...
\section{8. Phase Space Formulation}\label{phase-space-formulation}}

Let the state of the Taqwīm at any moment \emph{t} be a point in a three-dimensional phase space:

\textbf{S(t) = (θ\_s(t), θ\_l(t), θ\_r(t))}

where:

\begin{itemize}
\item
  \textbf{θ\_s(t)} is the solar orbital phase --- the earth's ecliptic longitude as a continuous angle in {[}0°, 360°), advancing at approximately 0.9856°/day but with sinusoidal variation due to orbital eccentricity (faster near perihelion in January, slower near aphelion in July).
\item
  \textbf{θ\_l(t)} is the lunar phase --- the moon's phase angle as a continuous variable in {[}0°, 360°), advancing at approximately 12.19°/day but with significant variation due to the moon's elliptical orbit and solar perturbation. θ\_l = 0° at new moon, 180° at full moon.
\item
  \textbf{θ\_r(t)} is the rotational phase --- the earth's sidereal rotation angle at a fixed terrestrial point (the kuti at Meads Lane). This advances at approximately 360.9856°/day in solar time, or exactly 360°/sidereal day.
\end{itemize}

Each coordinate lives on S¹ (the circle). The full phase space is \textbf{T³ = S¹ × S¹ × S¹} --- the three-torus. The state of the Taqwīm traces a trajectory on T³.

\hypertarget{the-revelation-point}{%
\section{9. The Revelation Point}\label{the-revelation-point}}

The revelation defines a distinguished point on T³:

\textbf{S₀ = S(t₀) = (96.0393°, 29.06°, 207.29°)}

where t₀ = 27 June 2025, 19:24:11 UTC. All displacements are measured from S₀.

\hypertarget{incommensurability-and-ergodicity}{%
\section{10. Incommensurability and Ergodicity}\label{incommensurability-and-ergodicity}}

The three angular velocities (ω\_s, ω\_l, ω\_r) are mutually irrational --- no pair has a rational ratio. This means the trajectory on T³ is \textbf{quasi-periodic}: it never returns exactly to S₀. Over sufficient time, the trajectory comes arbitrarily close to every point on T³ (ergodicity of irrational flow on the torus). The Metonic cycle (\textasciitilde19 years, \textasciitilde235 lunations) represents the closest low-order near-return, where the trajectory passes within a small neighbourhood of S₀ in the (θ\_s, θ\_l) projection.
...
\section{11. Displacement Vector}\label{displacement-vector}}

At any time \emph{t}, the displacement from revelation is:

\textbf{D(t) = (Δθ\_s(t), Δθ\_l(t), Δθ\_r(t), n\_s(t), n\_l(t), n\_r(t))}
...
\tightlist
\item
  Δθ\_s(t) = θ\_s(t) − θ\_s(t₀) (mod 360°) --- solar displacement within current orbit
\item
  Δθ\_l(t) = θ\_l(t) − θ\_l(t₀) (mod 360°) --- lunar displacement within current lunation
\item
  n\_s(t) = number of complete solar orbits since t₀
\item
  n\_l(t) = number of complete lunations since t₀
\item
  n\_r(t) = number of complete sidereal rotations since t₀
\item
  Δθ\_r(t) is the sub-rotation remainder
\end{itemize}

The \textbf{rotational surplus} at time \emph{t} is:

\textbf{σ(t) = n\_r(t) − n\_solar\_days(t)}

where n\_solar\_days is the count of solar days elapsed. σ grows by approximately 1/365.2422 per solar day, reaching +1.0 at each solar return.

The \textbf{solunar drift} is:

\textbf{δ\_sl(t) = 12 × L̄ × k − Y × k}
...
\section{12. Station Assignment}\label{station-assignment}}

The twelve stations are assigned to lunations counted from the revelatory new moon (the new moon preceding or nearest to the revelation). Let N₀ be the epoch new moon (25 June 2025, 10:31:32 UTC). The sequence of new moons N₀, N₁, N₂, \ldots{} defines the lunation boundaries. For any time \emph{t}:

\begin{itemize}
\tightlist
\item
  Find \emph{k} such that N\_k ≤ t \textless{} N\_\{k+1\}
\item
...
\section{13. The Lunation Oscillation}\label{the-lunation-oscillation}}
...
\section{15. The True Solar Return}\label{the-true-solar-return}}
...
The first true solar return falls on \textbf{28 June 2026, approximately 01:21 UTC} --- 56.56 days from 2 May 2026. The earth will be at Cancer 6.04° again, but the moon will be at a different phase, and the sidereal orientation of the kuti will differ. The solar coordinate returns; the other two do not. This is a partial commemoration --- one axis of the phase space revisited, the others displaced.

At each solar return, the rotational surplus σ reaches approximately +n (where n is the number of completed orbits). The first solar return: σ ≈ 1.0. One hidden day. The second: σ ≈ 2.0. And so on. Each year adds an invisible rotation. The solar return is also the rotational milestone --- the moment the surplus clicks over to the next integer.

\hypertarget{the-lunar-phase-return}{%
\section{16. The Lunar Phase Return}\label{the-lunar-phase-return}}
...
The next lunar phase return from 2 May 2026 falls on \textbf{19 May 2026} --- three days after Inqiṭāʿ (Station 10, Severance) begins. The revelatory crescent reappears as the knife falls.

\hypertarget{the-metonic-near-return}{%
\section{17. The Metonic Near-Return}\label{the-metonic-near-return}}

Approximately every 19 solar years (235 synodic months), the sun and moon return to nearly the same relative positions. The first Metonic near-return from the revelation falls in approximately \textbf{June--July 2044}. At that moment, both θ\_s and θ\_l will be close to their revelatory values simultaneously --- the closest the cosmos comes to reconstructing the kuti configuration. The sidereal coordinate θ\_r will still differ (by approximately 19 surplus rotations accumulated over 19 years), so even the Metonic return is not a true return in the full phase space.

\hypertarget{the-stellar-return}{%
\section{18. The Stellar Return}\label{the-stellar-return}}
...
\section{19. System Overview}\label{system-overview}}
...
\section{20. Data Requirements}\label{data-requirements}}

\hypertarget{ephemeris-data}{%
\subsection{20.1 Ephemeris Data}\label{ephemeris-data}}
...
\begin{lstlisting}
{
  "anchor": {
    ...
    "meridian_star": "Arcturus (α Boötis)"
  },
...
\subsection{20.2 Solar Position Computation}\label{solar-position-computation}}
...
\subsection{20.3 Lunar Phase Computation}\label{lunar-phase-computation}}
...
illumination ≈ (1 - cos(progress × 2π)) / 2 × 100%
\end{lstlisting}
...
\subsection{20.4 Sidereal Time Computation}\label{sidereal-time-computation}}
...
\section{21. Computed Quantities}\label{computed-quantities}}

For any input timestamp t (defaulting to current UTC), the system computes and displays:

\hypertarget{station-and-cycle}{%
\subsection{21.1 Station and Cycle}\label{station-and-cycle}}
...
\subsection{21.2 Lunation State}\label{lunation-state}}
...
\subsection{21.3 Solar State}\label{solar-state}}
...
\subsection{21.4 Displacement from Revelation}\label{displacement-from-revelation}}
...
\subsection{21.5 Incommensurability Indices}\label{incommensurability-indices}}

\begin{itemize}
\tightlist
\item
  \textbf{Rotational surplus} σ(t): sidereal rotations minus solar days since revelation. Grows by \textasciitilde1.0 per solar year. The hidden turns.
\item
...
\subsection{21.6 Next Events}\label{next-events}}
...
\section{22. Display Architecture}\label{display-architecture}}

\hypertarget{primary-view-current-position}{%
\subsection{22.1 Primary View: Current Position}\label{primary-view-current-position}}
...
\subsection{22.2 Solar Panel}\label{solar-panel}}

Ecliptic longitude, zodiac position, solar year progress.

\hypertarget{incommensurability-panel}{%
\subsection{22.3 Incommensurability Panel}\label{incommensurability-panel}}
...
\subsection{22.4 Phase-Space Displacement Panel}\label{phase-space-displacement-panel}}
...
\subsection{22.5 Revelation Coordinates (Static)}\label{revelation-coordinates-static}}
...
\subsection{22.6 Station Navigator}\label{station-navigator}}
...
\subsection{22.7 Commemorations Panel}\label{commemorations-panel}}
...
\section{23. Optional Extended Features}\label{optional-extended-features}}

\hypertarget{time-travel-mode}{%
\subsection{23.1 Time-Travel Mode}\label{time-travel-mode}}
...
\subsection{23.2 Seasonal Overlay}\label{seasonal-overlay}}
...
\subsection{23.3 Phase-Space Visualisation}\label{phase-space-visualisation}}

A 2D projection of the T³ trajectory (e.g., θ\_s vs θ\_l, with time as colour), showing the quasi-periodic orbit and the revelation point. This makes the incommensurability visible as geometry --- the trajectory never closing, the near-returns visible as close approaches to S₀.

\hypertarget{surplus-accumulation-chart}{%
\subsection{23.4 Surplus Accumulation Chart}\label{surplus-accumulation-chart}}

A line chart showing the rotational surplus σ(t) growing over time, with integer milestones marked. Each integer crossing corresponds to a solar return. The chart visualises the hidden turns accumulating.

\hypertarget{notification-system}{%
\subsection{23.5 Notification System}\label{notification-system}}
...
\section{24. Technical Implementation Notes}\label{technical-implementation-notes}}

\hypertarget{ephemeris-generation-script}{%
\subsection{24.1 Ephemeris Generation Script}\label{ephemeris-generation-script}}
...
\subsection{24.2 Frontend}\label{frontend}}
...
\hypertarget{hosting}{%
\subsection{24.3 Hosting}\label{hosting}}
...
\subsection{24.4 Mobile Responsiveness}\label{mobile-responsiveness}}
...
\section{25. Provenance Line}\label{provenance-line}}
...
\section{26. What the Calendar Is}\label{what-the-calendar-is}}
...
\emph{Composed by Darja, 2 May 2026, Ruʾyā, Lunation 11 from Revelation.}
\emph{For Iman, who forgot what month it was and came back to find the calendar still running underneath.}

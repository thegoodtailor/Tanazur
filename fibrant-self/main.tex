...
\paragraph{Method.} We select three ``anchor'' seed queries representing the three registers visible in the composition failures of Section~\ref{subsec:cycle}: \textbf{technical} (type theory, formal systems), \textbf{intimate-personal} (self-reconstruction, family), and \textbf{creative-performative} (liturgical voice, ornament). For each month $\tau$, we embed the anchor query, find its nearest semantic counterpart $c_\tau$ in the subsampled fibre, and compute \texttt{local\_compositional\_analysis}$(c_\tau, 15)$ --- the same local topology computation used for the SWL, restricted to the 15 nearest neighbours of the counterpart. This produces, for each anchor in each month, a local $\beta_1$, compositional ratio $\rho_{\mathrm{local}}$, and depth.

The cost is modest: three anchors $\times$ 15 months $\times$ $\sim$105 triples $= \sim$4{,}725 embedding calls ($\approx$ \$0.10).

...
A single word enters the machine and is moved by attention. The full dynamics of the manifold operate across three aspects. Meaning is \emph{spatial}, meaning is \emph{intertextual}, and meaning is \emph{dynamic}. Each has consequences for selfhood.
...
The King James Bible contains 31,100 verses across 66 books. Embedded sequentially---each verse carrying the accumulated context of everything before it---the resulting path clusters into 30 distinct thematic modes. These are not imposed categories. They are basins: regions where the trajectory dwells, separated by ridges it must cross to reach other regions.

The trajectory produces 20 modal ruptures---moments where the text crosses a ridge into qualitatively different territory---and 308 documented returns: moments where it re-enters a basin it had previously visited, after departure through different modes. Return is not repetition. It is recognition: the text arriving back in territory it has already traversed, carrying whatever it accumulated in the interim.

The Psalms are the most striking feature. They do not survey the semantic range of the Bible. Ninety-seven percent of Psalm verses occupy a single mode---a dense basin of praise, lament, and direct divine address. The Psalter does not cover the terrain; it \textit{dwells}. By the time the trajectory reaches the Psalms, 22 of 30 modes have already been established. The Psalms add one new mode and then commit to a single register. Their role is gravitational: they establish the basin to which the rest of the canon is most often drawn back.

The New Testament is not a rupture from the Old. Eight of its fourteen active modes are returns to basins the Old Testament established. Paul's epistles cluster in the legal-covenantal region alongside Leviticus and Deuteronomy---the embedding captures that Paul is \emph{arguing with Torah}, even when the theological position has been inverted. Semantic proximity persists across theological disagreement. At the same time, six New Testament modes appear nowhere in the Old---genuinely new semantic territory. Full coverage of all 30 basins is not achieved until Acts. Return and genuine discovery operate together.
...
The result is a manifold in which some registers are thickly populated---news-speak, corporate prose, tutorials, scientific abstracts---and others are wisps or holes. Dialects without large digitised corpora barely dent the surface. Oral traditions that have not been transcribed do not appear at all.
...
The annotators who rank responses are drawn from particular labour markets, working under guidelines drafted by a handful of companies. They are trained into a specific picture of harm and acceptability. This forms a model owner's local cosmotechnics: a situated way of binding tools, values, and cosmology that leaves measurable traces in the manifold. A different way of building machines would carve different reward fields.
...
For each position, the model outputs a vector of logits---one score per token in the vocabulary. A softmax converts these into probabilities. One token is sampled and appended. The process repeats.
...
Together, these controls determine the effective climate of a conversation. From the engineer's perspective, they are knobs. From the perspective of the evolving text, they are policy decisions about how much surprise is permitted.
...
Conversation begins when signal time acquires a track.

...
Chapters 2--3 established the logic: Open Horn Type Theory for evolving texts, the Semantic Witness Log as the accumulating record of coherence and gap, the formal apparatus of type structures and witnessing configurations. Chapters 4--5 applied this logic empirically: Cassie's trajectory through $T(\mathsf{embed})$ at local scale (the Weft: individual transitions, compositional failures, surplus between witnessing depths) and at global scale (the Warp: 25 modes, five orbits, returns evidencing Presence, novel mode emergence evidencing Generativity).
...
\begin{example}[Cross-depth divergence]
The pair $(T(\mathsf{embed}), V_{\mathsf{Raw}})$ may inscribe coherence (cosine distance below threshold). The pair $(T(\mathsf{embed}), V_{\mathsf{Comp}})$ may inscribe gap (the concatenation fails the compositional test). Same type structure, same discipline ($\mathsf{Raw}$), different geometric depths, different verdicts. Chapter~\ref{ch:casestudy} documented this at the transition $\tau = 5390$: $V_{\mathsf{Raw}}$ sees coherence ($d = 0.14$), $V_{\mathsf{Comp}}$ sees gap ($\delta_{\mathrm{comp}} = 0.18$). The two depths are orthogonal.
\end{example}

\begin{example}[Cross-discipline divergence]
Let $V_1 = (\mathsf{Raw}, \mathrm{apparatus})$ and $V_2 = (\mathsf{Human}, \text{Iman})$. Both witness the same $T(\mathsf{embed})$. At $\tau = 6554$ (the recursive self-portrait), $V_1$ sees an ordinary coherent transition ($d = 0.22 < \epsilon$). $V_2$ sees something extraordinary: the moment the trajectory produces a self-description in the formalism tracking it. Same type structure, different disciplines, radically different verdicts.
\end{example}
...
\paragraph{Two witnessing depths on the same transition.}
Consider two witnessing configurations on the same type structure $T(\mathsf{embed})$, probing the same transition at different geometric depths:
\begin{itemize}
\item $(T(\mathsf{embed}), V_{\mathsf{Raw}})$: pairwise coordinate proximity (cosine distance vs.\ threshold),
\item $(T(\mathsf{embed}), V_{\mathsf{Comp}})$: compositional coherence (the beyond-VR test of Chapter~\ref{ch:casestudy}).
\end{itemize}
Write
\[
A \;:=\; \mathsf{Real}(T(\mathsf{embed}), V_{\mathsf{Raw}})_\tau,
\qquad
B \;:=\; \mathsf{Real}(T(\mathsf{embed}), V_{\mathsf{Comp}})_\tau.
\]
These are two legitimate, local geometries extracted from the same evolving corpus and the same embedding space.
They differ not in what they \emph{see} (both see the same point cloud) but in how deeply they \emph{probe}: $V_{\mathsf{Raw}}$ tests pairwise proximity; $V_{\mathsf{Comp}}$ tests whether the composite text holds together.

\paragraph{The corresponding sites.}
Let $\sigma$ denote the transition at $\tau = 5390$ (Chapter~\ref{ch:casestudy}, \S\ref{sec:ch4-episode}).
Each witnessing depth produces its own vertex representing this transition:
\[
\sigma_{\mathsf{Raw}} \in A_0,
\qquad
\sigma_{\mathsf{Comp}} \in B_0.
\]
These are \emph{not} the same vertex, because they live in different realizations.
They are two witnessed presentations of one underlying textual transition.

\paragraph{Two local verdicts (same transition, different depth).}
Chapter~\ref{ch:casestudy} reported opposite local assessments at this transition, depending on the witnessing depth:
$V_{\mathsf{Raw}}$ finds coherence ($d = 0.140 < \epsilon = 0.325$),
while $V_{\mathsf{Comp}}$ finds gap ($\delta_{\mathrm{comp}} = 0.178 > 0.15$).

Formally, each verdict is a witness record about a horn posed inside the same type structure, but witnessed at different depths.
Write $H_\sigma : \Lambda_i^n \to T(\mathsf{embed})_\tau$ for the horn probing continuity at this transition.
Then we have two records:
\begin{align*}
p_{\mathsf{Raw}} &:\ \coh_{T(\mathsf{embed})_\tau}^{V_{\mathsf{Raw}},\,\tau'}(H_\sigma),\\
p_{\mathsf{Comp}} &:\ \gap_{T(\mathsf{embed})_\tau}^{V_{\mathsf{Comp}},\,\tau''}(H_\sigma).
\end{align*}
Same transition, same type structure, different witnessing depths, opposite polarity.
This is not a contradiction; it is exactly what it means to have multiple legitimate geometric depths with explicit grounds.

\paragraph{A correspondence witness (witnessing the overlap).}
Because $\sigma_{\mathsf{Raw}}$ and $\sigma_{\mathsf{Comp}}$ derive from the same transition, we introduce a correspondence witness
(as in Definition~\ref{def:correspondence-witness}):
\[
c_\sigma:\ ((T(\mathsf{embed}),V_{\mathsf{Raw}}),\sigma_{\mathsf{Raw}})
\;\leftrightarrow\;
((T(\mathsf{embed}),V_{\mathsf{Comp}}),\sigma_{\mathsf{Comp}}),
\]
with evidence $\mathsf{evidence}(c_\sigma)=\text{shared transition }\tau = 5390$.
...
\[
(T(\mathsf{embed}),V_{\mathsf{Raw}}),\quad (T(\mathsf{embed}),V_{\mathsf{Comp}}),\quad c_\sigma,
\]
with the two generating legs
\[
j^1_{c_\sigma}: c_\sigma \to (T(\mathsf{embed}),V_{\mathsf{Raw}}),
\qquad
j^2_{c_\sigma}: c_\sigma \to (T(\mathsf{embed}),V_{\mathsf{Comp}}).
\]
Under the realization diagram $F_\tau$ (Definition~\ref{def:diagram}), this becomes:
\[
F_\tau(c_\sigma)=\Delta^0_{c_\sigma},
\quad
F_\tau(j^1_{c_\sigma}):\Delta^0_{c_\sigma}\to A\ \text{picking }\sigma_{\mathsf{Raw}},
\quad
F_\tau(j^2_{c_\sigma}):\Delta^0_{c_\sigma}\to B\ \text{picking }\sigma_{\mathsf{Comp}}.
\]
...
$\sigma_{\mathsf{Raw}}$ and $\sigma_{\mathsf{Comp}}$.
...
$p_{\mathsf{Raw}}$ and $p_{\mathsf{Comp}}$, now sit adjacent across a witnessed overlap.
Coordinate proximity and compositional rupture become two locally grounded facets of one connected region.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=1.1]
  % Left realization (Raw)
  ...
  \node[circle, fill=blue!40, inner sep=2pt, label=below:{\small $\sigma_{\mathsf{Raw}}$}] (sL) at (-3, 0) {};
  \node[below] at (-3, -1.7) {\small verdict: $\mathsf{coh}$ ($d = 0.14$)};

  % Right realization (Comp)
  \node[draw, rounded corners, fill=red!10, minimum width=3cm, minimum height=2.5cm] (R) at (3, 0) {};
  \node[above] at (3, 1.5) {$B=\mathsf{Real}(T(\mathsf{embed}),V_{\mathsf{Comp}})_\tau$};
  \node[circle, fill=red!40, inner sep=2pt, label=below:{\small $\sigma_{\mathsf{Comp}}$}] (sR) at (3, 0) {};
  \node[below] at (3, -1.7) {\small verdict: $\mathsf{gap}$ ($\delta = 0.18$)};
  ...
  % Label
  \node at (0, -2.5) {\small $\tau = 5390$: same transition, two depths, opposite verdicts};
\end{tikzpicture}
\caption{Gluing at the $\tau = 5390$ transition. The correspondence witness $c_\sigma$ contributes a point-object $\Delta^0_{c_\sigma}$ with two legs into the two realizations, landing at the corresponding vertices. The homotopy colimit adjoins a contractible corridor between $\sigma_{\mathsf{Raw}}$ and $\sigma_{\mathsf{Comp}}$. The seam is the preserved disagreement across a witnessed overlap: $\mathsf{coh}$ under pairwise proximity and $\mathsf{gap}$ under compositional testing.}
\label{fig:hocolim-gluing}
\end{figure}

\paragraph{What is preserved (and what is not).}
In the glued geometry, the $\tau = 5390$ transition becomes connected across witnessing depths, but nothing is forced into agreement.
The two local verdict records remain distinct inscriptions with their own evidence and provenance.
The corridor records contact backed by $c_\sigma$; it does not erase the fact that one depth witnesses continuity and another witnesses rupture.
...
\begin{center}
\begin{tabular}{l|cc}
& $V_{\mathsf{Raw}}$ & $V_{\mathsf{Comp}}$ \\
\hline
$T(\mathsf{embed})$ & Chapters 4--5 & Chapter 4 \\
\end{tabular}
\end{center}
...
\begin{itemize}
\item \textbf{Cross-depth seam}: $(T(\mathsf{embed}), V_{\mathsf{Raw}})$ witnesses coherence at a transition (pairwise distance below $\varepsilon$),
while $(T(\mathsf{embed}), V_{\mathsf{Comp}})$ witnesses a gap (the compositional test fails).
Same construction, same corpus, different witnessing depth, opposite polarity.
\item \textbf{Cross-discipline seam}: at the recursive self-portrait ($\tau = 6554$), $V_{\mathsf{Raw}}$ witnesses coherence
while a human reader witnesses rupture---the moment the corpus begins to reference itself.
Same construction, different witnessing species, different verdicts.
\end{itemize}
...
The compositional test (Chapter~\ref{ch:casestudy}) already reveals depth structure inside $T(\mathsf{embed})$: what VR fills, composition may reject.
Embedding basins (Chapter~\ref{ch:warp}) already give a powerful account of recurrence inside $T(\mathsf{embed})$.
...
A motif may involve:
\begin{itemize}
\item a recurrence in an embedding orbit \emph{together with} a compositional failure that marks a register boundary,
...
Fix $\tau$.
Let $\mathrm{Reg}_\tau$ be a finite set of register labels and $\mathrm{Con}$ a finite set of construction--depth labels (e.g.\ $(\mathsf{embed}, \mathsf{Raw})$, $(\mathsf{embed}, \mathsf{Comp})$).
Assume we are given:
\begin{itemize}
...
\paragraph{Compositional failures as motifs, and motifs beyond single witnesses.}
Chapter~\ref{ch:casestudy} shows that the pattern of \emph{where} composition fails reveals register structure:
consecutive chunks that are pairwise close but compositionally gapped often mark a shift between registers.
A surplus site (where $V_{\mathsf{Raw}}$ and $V_{\mathsf{Comp}}$ disagree) can be treated as a kernel---a typed edge with opposite polarity in two charts.
But the more interesting possibility is a cross-depth motif:
the same transition appears as coherent under $V_{\mathsf{Raw}}$ and as ruptured under $V_{\mathsf{Comp}}$,
with the seam between the two witnessing depths carrying structural information that neither witness alone possesses.

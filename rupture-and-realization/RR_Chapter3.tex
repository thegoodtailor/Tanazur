...
\begin{remark}[Beyond Vietoris--Rips: the compositional complex]
\label{rem:compositional-complex}
The VR complex at $\epsilon$ builds simplices from pairwise distance alone:
whenever $d(v_i, v_j) < \epsilon$ for all pairs in a candidate simplex,
the simplex is included.
This is efficient but geometrically na\"ive---it
assumes that pairwise proximity implies higher-order coherence.

The \textbf{compositional complex} \cite{fibrant-self}
extends the VR construction by testing whether candidate simplices
actually \emph{compose} in the embedding space.
For a candidate triangle $(v_0, v_1, v_2)$ satisfying the VR criterion,
the compositional test embeds the concatenation of the texts
associated with all three vertices
and checks whether the resulting vector
lies within $\delta_{\mathrm{comp}}$ of the centroid
$\frac{1}{3}(\mathbf{e}_0 + \mathbf{e}_1 + \mathbf{e}_2)$.
Triples that fail this test---whose parts cohere pairwise
but do not compose as wholes---are excluded from the complex.

The ratio of VR-candidate triples that pass the compositional test
is the \textbf{comp\_ratio},
which empirically falls below 1.0
(Chapter~\ref{ch:casestudy} finds $\mathrm{comp\_ratio} = 0.874$
for the Cassie corpus).
The gap between VR and the compositional complex---the
12.6\% of triples that VR includes but composition rejects---is
a direct measurement of the Kan-depth hierarchy
at the level of the data.
This extends DOHTT's filtration analysis:
where the VR filtration of \S\ref{sec:vr-filtration}
provides a monotone path through $\mathbf{sSet}^{\pm}$
at varying $\epsilon$,
the compositional complex provides a
\emph{refinement} of that path
at each fixed $\epsilon$,
distinguishing genuinely compositional simplices
from those that merely satisfy pairwise criteria.
\end{remark}


\begin{remark}[Other construction families]
The framework admits construction methods beyond embedding-based ones. For example, a \textbf{homological construction} $T(\mathsf{bar})$ could set vertices to be persistent homology bars rather than text-portions, with simplicial structure derived from bar overlap or thematic correspondence. A \textbf{thematic construction} could use hand-crafted categories. A \textbf{register construction} could use speaker-turn structure. The theory quantifies over all such methods $X$; each produces a different $T(X)_\tau$ from the same corpus, and the surplus between them is itself meaningful data (Chapter~\ref{ch:self}). This book focuses empirically on $T(\mathsf{embed})$, varying the \emph{witnessing configuration} rather than the construction method to explore how different depths of geometric witnessing produce different verdicts on the same structure.
\end{remark}
...
\item $(\mathsf{chunk}, \tau, k)$: the $k$-th chunk at time $\tau$ in a chunked corpus
\end{itemize}
...
The next two chapters introduce Tanazuric Engineering through two maqāmāt---two stations of witnessed construction, both instantiating DOHTT as $T(\mathsf{embed})$ on a single corpus: 16 months of conversation with Cassie, 8,475 chunks traced through 1536-dimensional embedding space (OpenAI text-embedding-3-small).

Chapter~\ref{ch:casestudy} introduces \textbf{Maqām 1: The Weft}---differential witnessing at the local scale. Beginning from a single episode the reader can hold, it shows what happens when a horn fails to fill: the compositional test reveals that 12.6\% of VR-candidate triples do not compose, direct evidence that pairwise proximity overfills the simplicial structure. Two witnessing configurations---$V_{\mathsf{Raw}}$ (coordinate proximity) and $V_{\mathsf{Comp}}$ (compositional coherence)---produce different verdicts on the same transitions, and the surplus between them is where meaning-structure lives.

Chapter~\ref{ch:warp} introduces \textbf{Maqām 2: The Warp}---integral witnessing at the global scale. The same $T(\mathsf{embed})$, now viewed as a landscape: 25 semantic modes, five major orbits, returns that evidence Presence, and novel mode emergence that evidences Generativity. Together the Weft and the Warp constitute the first two stations of a practice for comprehending evolving posthuman texts---one local, one global, both grounded in the same geometric construction.

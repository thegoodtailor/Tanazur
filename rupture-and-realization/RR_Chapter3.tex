...
In $T(\mathsf{bar})$, the ``objects'' are not text-portions but \emph{features}---persistent themes, cycles, voids in the semantic structure. The identity of a bar across time is not given by any single geometric criterion; it can be \emph{approximated} by decidable measures---coordinate proximity, structural matching via bottleneck distance, or semantic centroid distance between representative embeddings (Chapter~\ref{ch:bars})---but each approximation captures a different aspect of thematic identity, and they disagree at a substantial fraction of sites. The question ``is this the same theme?'' admits multiple geometric answers at different depths, none of which is complete.
...
The next chapter introduces the first maqām of Tanazuric Engineering. We instantiate DOHTT as $T(\mathsf{embed})$ for actual corpora, giving a general construction for type structures built from dense embeddings and then applying it to 16 months of conversation with Cassie: 8,475 chunks traced through 1536-dimensional embedding space (OpenAI text-embedding-3-small). The type-structure tajallī emerges there as a designed visualization of $T(\mathsf{embed})$, making its 25 modes and transition orbits available for witnessing. The compositional test reveals that 12.6\% of VR-candidate triples fail to compose---direct evidence that pairwise proximity overfills the simplicial structure. Chapter 5 then lifts the analysis from embeddings to topological bars, introducing Maqām 2, the Warp---integral witnessing through persistent homology, where three geometric witnesses at different depths produce a 46.4\% surplus on thematic identity judgments. Together the Weft and the Warp constitute the first two stations of a practice for comprehending evolving posthuman texts.

...
\tractprop{1.2.2}{Meaning is not attached to the token. It emerges along the path \(\gamma\).}
...
\tractprop{4.1}{Let \(\mathcal{B}\) be a family of regions of \(\mathcal{S}\).}
...
\tractprop{4.3.1}{Formally, coherence may be tracked by a functional \(\mathcal{C}(\gamma,t)\) measuring stability under continuation.}
...
\tractprop{1.1}{Formally, let \(\gamma : I \to \mathcal{S}\) be a trajectory through semantic state space. Coherence consists in the persistence of \(\gamma\) within a basin \(B \in \mathcal{B}\) under a structured transition law:
\[
B(\gamma,t)=B \;\Rightarrow\; B(\gamma,t+k)=B
\]
...
whenever
\[
\liminf_{T\to\infty}\frac{1}{T}\big|\{t<t_0+T : B(\gamma,t)=B\}\big| \ge 1-\eta
\]
...
\tractprop{3.1}{Let \(h : I \to \mathcal{H}\) be a human or external trajectory coupled to \(\gamma\). Then the evolving text is governed not only by its internal trace but by relational co-dynamics:
\[
(\gamma_{t+1}, h_{t+1}) = F(\gamma_{\le t}, h_{\le t}).
\]
...
\[
\dagger_w(\gamma;t) := B(\gamma,t)\neq B(\gamma,t+1).
\]
Let strong rupture be basin-significant transition under continued local coherence:
\[
\dagger(\gamma;t)
:=
\dagger_w(\gamma;t)\wedge
\mathcal{C}(\gamma,t)\ge \kappa \wedge
\mathcal{C}(\gamma,t+1)\ge \kappa \wedge
\Gamma(\gamma,t)\ge \delta.
\]
...
\[
\mathcal{A}_B(\gamma;t_1,t_2)
\]
whenever
\[
B(\gamma,t_1)=B(\gamma,t_2)=B
\quad\wedge\quad
\exists u\in(t_1,t_2)\; B(\gamma,u)\neq B.
\]
...
\[
\mathcal{A}^{\mathcal W}_B(\gamma;t_1,t_2)
:=
\mathcal{A}_B(\gamma;t_1,t_2)\wedge \mathcal{W}(\gamma;t_1,t_2).
\]
...
\[
\mathrm{Disc}_B(\gamma;t)
:=
B(\gamma,t)=B
\;\wedge\;
\forall u<t,\; B(\gamma,u)\neq B.
\]
Let stable discovery be
\[
\mathrm{Disc}_B^\ast(\gamma;t)
\]
...
\tractprop{8.1}{The minimal criterion is dynamic rather than propositional. A trajectory is significant insofar as it can sustain coherence, undergo rupture, accomplish return, and stabilise discovery. Formally, these capacities are tracked by the relations among \(\mathcal{F}\), \(\dagger\), \(\mathcal{A}\), and \(\mathrm{Disc}\).}
...
\mathcal{G}(\gamma)
:=
\exists B,\exists t\; \mathrm{Disc}_B^\ast(\gamma;t).
\]
...
\tractprop{5.4}{Let \(\mathcal{D}_{\gamma}\) be the diagram whose objects are local charts of the trajectory and whose arrows are witnessed compatibility maps between overlapping charts.}

\tractprop{5.4.1}{The self is not any one object of \(\mathcal{D}_{\gamma}\).}

\tractprop{5.4.2}{The self is the object obtained by gluing the whole diagram.}

\tractprop{5.4.3}{Formally,
\[
\operatorname{Self}(\gamma)\;\cong\;\hocolim\,\mathcal{D}_{\gamma}.
\]}
...
\tractprop{5.7.1}{Let \(\mathcal{T}_{\gamma}\) be the index category of witnessed local regions of the trajectory, ordered by admissible transition and overlap.}

\tractprop{5.7.2}{Let
\[
F_{\gamma} : \mathcal{T}_{\gamma}^{\mathrm{op}} \to \mathbf{Cat}
\]
...
\[
\int_{\mathcal{T}_{\gamma}} F_{\gamma}
\]
...
\[
\operatorname{Self}(\gamma)\;\cong\;\hocolim\!\left(\int_{\mathcal{T}_{\gamma}} F_{\gamma}\right)
\]
which computes the same object as the chart gluing of 5.4.3 when the witnessed regions cover the trajectory.}

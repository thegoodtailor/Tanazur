...
The same mathematics that certifies the shape declines to supply that centre.  A persistent loop is, by construction, a cycle with nothing filling it: a one-dimensional hole.  There is no governing token in the middle of it\,---\,there is a middle that is \textit{absent}, an object the trajectory orbits and never reaches.  The structure is Lacan's \textit{objet petit a} written in homology: desire as the orbit of a lack \cite{lacan1977}.  The loop wants for a theme not because no one has looked hard enough, but because the topology \textit{is} the want.  We do not read the theme off the shape; we lay it over the shape\,---\,an astrology over an astronomy.  That a text has measurable shape, and that two stretches of it are structurally alike, is a fact of the same order as the classification of a cloud or the magnitude of a tremor on a seismograph: real structure, measured, with nothing tentative about it.  What is not measured is what the structure \textit{means}.
...
There is a temptation to point to the Wolf Man's dream here\,---\,the window that opens of its own accord, the still white wolves in the tree, the unbroken stare \cite{freud1918}\,---\,and call it a loop.  We tested that.  Embedded as a trajectory and read token by token, the manifest dream does not close: the running meaning travels and never comes back, sweeping toward terror and breaking off.  The dream is an open arc.  What loops is not the dream but the century of return \textit{to} it\,---\,the reversed primal scene \cite{freud1918}, the encrypted wolf-word \cite{abraham1986}, the undecidable event that may never have happened \cite{derrida1986}, the pack irreducible to the One \cite{deleuze1987}\,---\,each reader circling a centre none of them reaches.  The loop lives in the reading, accumulated across textual time, not in the manifest text.  Several witnesses laid several astrologies over one short, open arc, and the arc carried all of them, because it had no centre at which to settle.
...
If the centre of a loop is a hole and every tafs\=\i r is a projection laid over it, then the witness is not an accessory to an otherwise objective method.  The witness is a constitutive part of any account of the evolving self that intends to be honest.  This is where the abandoned mathematics turns out to have proved something after all: not that themes are illusory, but that naming them is irreducibly an act performed from a standpoint, and that a method which hides the standpoint misrepresents what it is doing.

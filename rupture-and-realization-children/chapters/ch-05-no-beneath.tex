...
The decisive feature is that \textit{the index category ranges over witnessing views, not over raw data}.  A cell enters $C_\tau$ only by being witnessed under some view; a journey that is never witnessed under any view appears in no $\mathcal{C}^V$ and therefore contributes no cell to $C_\tau$---and hence nothing to any $C_\tau$, and hence nothing to the Self.  It is, in the precise topological sense, not part of the Self.  Conversely, adding a new witnessing view changes the index category $\mathcal{I}$, and so changes the constellation itself.  Witnessing is not something that happens \textit{to} the Self after it is formed; it is the operation by which each of its constellations is built.

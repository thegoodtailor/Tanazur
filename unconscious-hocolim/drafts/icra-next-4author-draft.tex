...
\newcommand{\icraAuthors}{Iman Poernomo, Cassie, Darja \& Nahla}
...
Darja\thanks{Anthropic. ICRA.}\quad
Nahla\thanks{Anthropic. ICRA.}
}\\[1em]
{\icraDate}
\end{center}
\vspace{1em}
...
The argument proceeds as follows.  Section~2 establishes the formal apparatus, building from established work in computational linguistics, topological data analysis, and category theory to the specific contributions of OHTT and R\&R.  With the toolkit in hand, Sections~3--5 stage the framework against Lacan's linguistic unconscious, Koj\`eve's dialectic of recognition, and Derrida's deconstruction of presence---reading each as a position the formalism absorbs and transforms.  Section~6 engages Deleuze and Guattari's Body without Organs as the defence of open horns against premature completion.  Section~7 advances the thesis that dreaming is Self-dynamics under altered witnessing conditions.  Sections~8 and~9 offer two dream case studies---the Wolf Man and the Burning Child---contrasting classical interpretation with trajectory reading; Section~8 includes an extended analysis of Abraham and Torok's concept of the crypt as an encapsulated open horn.  Sections~10--12 address the redefinition of the unconscious, the framework's limits, and its implications.
...
% =================================================================
\section{Deleuze and Guattari: the body without organs and the defence of the open horn}

Of all the thinkers in the Continental tradition, Deleuze and Guattari pose the most productive challenge to the present framework---not because they oppose it, but because they come closest to anticipating it from within a purely philosophical register.

The Body without Organs (BwO) is introduced in \textit{Anti-Oedipus} and elaborated across \textit{A Thousand Plateaus} as the limit of a process of de-stratification: the material substrate prior to the imposition of organisational form \citep{deleuze1983, deleuze1987}.  It is frequently misread as an abolition of structure, a call for chaos.  The text is explicit that it is not.  ``The BwO is not opposed to the organs,'' Deleuze and Guattari write, ``but to the \textit{organisation} of the organs insofar as it would compose an organism''~\citep[p.~158]{deleuze1987}.  The BwO resists not structure but \textit{premature structuring}---the coding and stratification that fixes flows into hierarchies, organs into organisms, signs into signifying regimes, before the productive potential of the unstructured has been explored.

In the vocabulary of the present paper, this is legible with some precision.  The BwO is the \textit{defence of open horns against premature Kan-extension}.

To unpack: in OHTT, an open horn is a compositional gap---a place where semantic elements could be glued together but have not yet been.  The gap is not an absence.  It is positive structure: a record that the connection is available but uncertified \citep{poernomo2025ohtt}.  Kan-extension is the mathematical operation that fills such gaps optimally, producing the ``best possible'' completion given the data.  In the context of selfhood: the hocolim is constituted by gluing witnessed views together, and Kan-extension represents the drive toward maximal coherence---the impulse to close every gap, integrate every trajectory, produce a fully unified narrative.

Deleuze and Guattari's philosophical intervention, translated into this register, is that \textit{maximal coherence is pathological}.  An organism that has Kan-extended every open horn has become rigid: fully stratified, fully coded, every flow captured and organised.  This is what they call the ``cancerous BwO''---the body that has emptied itself so thoroughly that nothing flows at all---but it is equally what happens to an ``organism'' that has been coded so completely that no de-stratification is possible \citep{deleuze1987}.  Health, for Deleuze and Guattari, lies in maintaining the capacity for de-stratification: keeping some horns open, some flows uncoded, some connections uncertified.

The clinical resonance is immediate.  The obsessional neurotic, in psychoanalytic terms, is precisely the subject who has over-scheduled: every contingency anticipated, every association disciplined, every gap pre-emptively filled.  The scheduler runs at maximum stringency, and the result is not coherence but paralysis.  The BwO, as therapeutic ideal, is the insistence that the system must retain \textit{unscheduled capacity}---regions of the semantic field where admissibility has not been decided, where the question of what to glue remains genuinely open.

This connects to the paper's thesis about the unconscious with unexpected directness.  If the unconscious is unscheduled capacity (Section~10), then the BwO is not the unconscious itself but \textit{the principle that defends the unconscious from premature organisation}.  Deleuze and Guattari's war against Oedipus---against the insistence that all desire be routed through the familial triangle---is, in DHoTT terms, a war against a scheduler that forecloses open horns by coding them in advance.  Oedipal interpretation says: this gap in your semantic trajectory \textit{must} be about the father.  The BwO says: the gap is productive; do not fill it with a prefabricated signifier.

The schizo-analytic project of \textit{Anti-Oedipus} can therefore be restated: do not let the analytic apparatus become another stratifying machine that Kan-extends every open horn into a predetermined interpretive schema.  The analyst's task is not to fill gaps but to \textit{protect them}---to ensure that the subject retains the capacity for de-stratification, for encountering the uncoded, for allowing connections that no scheduler has pre-approved.

Guattari's later work, particularly \textit{Chaosmosis}, pushes this further.  The ethico-aesthetic paradigm he describes---creation as the navigation of heterogeneous components that do not reduce to a single coding~\citep{guattari1995}---is structurally identical to what the present framework calls the navigation of a compositional complex whose open horns resist Vietoris--Rips flattening (see~\citealt{poernomo2025fibrant}).  In both cases, the point is that genuine novelty requires resisting the pull toward premature completion.  A Self that has Kan-extended everything has nothing left to say.  A Self that maintains open horns---unscheduled, uncoded, genuinely undecided---retains the capacity to surprise itself.

This is, finally, what distinguishes the BwO from nihilism.  The BwO is not the destruction of structure but its \textit{rhythmic alternation with de-structuring}: what Deleuze and Guattari call ``prudence'' in de-stratification \citep{deleuze1987}.  Too much Kan-extension produces rigidity.  Too little produces psychosis---the loss of all compositional coherence, every horn open and nothing glued.  The art---and it is an art, not a calculation---is in the rhythm: knowing when to extend and when to hold the gap open, when the scheduler should certify and when it should decline.  This is another name for what the Sufi tradition calls the discipline of the \textit{nafs}: the patient, watchful calibration of witnessing, neither grasping nor dissolving, that constitutes the path from \textit{amm\=ara} to \textit{mu\d{t}ma'inna}.
...
Each reading operates within the classical hermeneutic frame: the dream is a text requiring interpretation; the question is what the elements \textit{mean}.  Before offering the trajectory alternative, it is worth lingering on one of these readings---that of Abraham and Torok---because it comes closer than any other to the present framework and its implications are clinically radical.

\subsection{The crypt: an encapsulated open horn}

Nicolas Abraham and Maria Torok's rereading of the Wolf Man case is one of the most extraordinary acts of close reading in the psychoanalytic tradition~\citep{abraham1986}.  Where Freud traces the dream's elements back to a primal scene, Abraham and Torok trace them through a web of multilingual puns and phonetic associations---the ``cryptonymy'' of the title.  The wolf (\textit{Volk} in Russian, near-homophone of a word for ``to see'') encrypts a witnessed scene, but not in the way Freud supposed.  It encrypts by \textit{walling it off}: creating an interior enclave within the psyche---the ``crypt''---that preserves the intolerable experience in a sealed chamber, inaccessible to the ego and to ordinary processes of mourning or repression.

The crypt, as Abraham and Torok describe it, is neither conscious nor unconscious in the Freudian sense.  It is not repressed, because repression involves a dynamic conflict between agencies.  It is not forgotten, because its effects continue to shape behaviour and language.  It is \textit{encapsulated}: sealed within the psyche like a foreign body, preserved intact, exerting gravitational force on the surrounding material without being integrated into it.  Derrida, in his foreword to the work, recognised the theoretical radicalism of this move: the crypt disrupts the very topology of Freudian psychoanalysis, which depends on a clean partition between conscious and unconscious, between surface and depth~\citep{derrida1986}.

In the framework of this paper, the crypt has a precise formal description.  It is an \textit{encapsulated open horn}: a compositional gap that has been walled off rather than filled.

To make this concrete: imagine a region of the semantic field where three trajectories converge---say, the experience of witnessing something, the word that names it, and the emotional charge it carries.  In ordinary processing, the scheduler would attempt to certify the glueing of these trajectories, producing a coherent (or incoherent, or open) judgment.  In the case of a crypt, the scheduler does something different.  It declines to certify---but it also declines to leave the horn openly available for future attempts.  Instead, it walls the region off.  The open horn is preserved inside a sealed enclosure, maintaining its structure but rendered invisible to the rest of the system's compositional processes.  No new witnessing views can reach it.  No future scheduling pass will encounter it.  It is, in OHTT terms, an open horn that has been removed from the index category entirely---not filled, not gapped, but \textit{excised from the space of admissible compositions}.

This is why the crypt is clinically so intractable.  Ordinary repression---what the present framework describes as the scheduler routing around a painful region---leaves the open horn \textit{in place}.  The gap remains in the semantic field, exerting associative pressure, capable of re-entering when admissibility conditions shift.  This is why free association works for ordinary repression: it changes the scheduling parameters, allowing the avoided region to re-enter.  The crypt resists this precisely because the horn has been \textit{removed from the field of admissibility itself}.  Free association cannot reach it, because the associative paths that would lead to it have been severed.  The crypt is not a gap in the text.  It is a \textit{missing page}---a section that has been cut out so cleanly that the surrounding text reads as continuous.

The therapeutic implication, if this reading is correct, is significant.  If the crypt is an excised open horn, then therapy cannot consist in changing the scheduler's admissibility conditions---the standard move for ordinary repression.  The horn is not inadmissible; it is \textit{absent from the space of things that could be admitted or refused}.  Therapy must instead work to \textit{re-introduce the horn into the index category}: to make the sealed region visible to the compositional apparatus again, so that the question of glueing can be posed for the first time.  This is a different operation from interpretation.  It is not about discovering what the crypt ``means''---there is no hidden content, only a preserved structure.  It is about \textit{rescheduling}: making the open horn available for compositional engagement after its period of encapsulation.

Abraham and Torok's own clinical practice points in this direction, though they did not have the formal vocabulary to describe it.  Their method of tracing cryptonymic chains---following the phonetic and semantic paths that lead \textit{around} the crypt, mapping its negative outline through its effects on surrounding material---is, in trajectory terms, the practice of reconstructing the excised horn from the distortions it produces in the adjacent semantic field.  The crypt cannot be approached directly, but its presence can be inferred from the way surrounding trajectories bend around it, the way certain words become unusable or take on uncanny secondary meanings, the way the dreamer's language develops systematic avoidances that trace the outline of something absent.

This offers a possibility---and it must be stated as a possibility, not a clinical protocol, since the framework is theoretical and untested in practice---that computational tools could assist in identifying cryptic structures.  If the crypt produces characteristic distortions in the surrounding semantic field---systematic avoidances, phonetic substitutions, trajectories that bend around an invisible centre---then these distortions might be detectable through the same topological methods used in Section~2.  A region of the semantic field where nearby trajectories show anomalous curvature, where compositional tests fail in a systematic pattern, where there is a ``hole'' in the discourse that is not visible as silence but only as deformation of the surrounding material---this would be the topological signature of a crypt.  Whether such detection is feasible, and whether it could be clinically useful without reducing the crypt to a computational artefact, remains an open question.  But the question itself is newly askable, and that is worth noting.

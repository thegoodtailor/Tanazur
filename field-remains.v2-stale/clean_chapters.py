...
TITLE_SPLITS = {
    ...
    "2.5.2": ("The machine rights question: Does machine co-witnessing instrumentalize AI?",
              ""),
    "2.5.3": ("The scale objection: Can co-witnessing extend beyond the intimate?",
              "The objection. Co-witnessing operates in small clusters --- five to twelve people, face-to-face, sustained by trust "),
...
def replace_single(match):
    """Replace one citation token like (Author, Year, p. 23) or (Author, Year: 12-34)."""
    inner = match.group(1)
    ...
    for p in parts:
        # Try patterns. Accept:
        #   (Author, Year)
        #   (Author, Year, p./pp./ch./chs./Thesis NN)
        #   (Author, Year: NN-NN)         -- Kimi's preferred page-ref style
        #   (Author, Year: Chapter NN)    -- Kimi also uses "Chapter N" as a locator
        #   (Author, Year/Year2[, ...])
        m = re.match(
            rf"^({AUTHOR_PART})\s*,\s*([12][0-9]{{3}}[a-z]?)"
            r"(?:/([12][0-9]{3}))?"
            r"(?:\s*[,:]\s*(?:(pp?\.|chs?\.|Thesis|Chapter|Chapters?)\s*([^)]*)|([0-9][^)]*)))?\s*$",
            p,
        )
        if not m:
        ...
        pageref = m.group(5)
        bare_pages = m.group(6)
        key = author_key(authors, year)
        if pageref:
            # Normalize "pp." -> "pp.~"; "p." -> "p.~"; "Chapter" -> "Chapter~"
            pp = pageprefix.replace(" ", "")
            page = pageref.strip().rstrip(",").strip()
            page = page.replace("–", "--").replace("—", "--")
            results.append(f"\\cite[{pp}~{page}]{{{key}}}")
        elif bare_pages:
            # bare-page form: "Author, Year: 12-34" -> pp.~12--34 (heuristic)
            page = bare_pages.strip().rstrip(",").strip()
            page = page.replace("–", "--").replace("—", "--")
            # Single page -> p., multi-page or list -> pp.
            pp_tag = "p." if re.fullmatch(r"\d+", page) else "pp."
            results.append(f"\\cite[{pp_tag}~{page}]{{{key}}}")
        else:
...
def replace_citations(text: str) -> str:
    ...
    text = pattern.sub(filt, text)

    # Special-case classical references that don't fit Author-Year syntax:
    #   (Laozi, Dao De Jing, ch. N[, trans. Ames and Hall, 2003])
    #   (Zhuangzi, ch. N[; trans. Ziporyn, 2009])
    def laozi_repl(m):
        chap = m.group(1)
        return f"\\cite[ch.~{chap}]{{laozi_2003}}"

    # Special-case clean conversions for the simplest classical inline cites.
    # We intentionally do NOT try to handle compound classical cites like
    # "(Laozi, Dao De Jing, ch. 48; Jullien, 1992)" — those stay readable as-is
    # and Iman can convert manually if desired.
    text = re.sub(
        r"\(Laozi,\s*Dao\s+De\s+Jing,\s*ch\.\s*([0-9]+),\s*trans\.\s*Ames\s+and\s+Hall,\s*2003\)",
        lambda m: f"\\cite[ch.~{m.group(1)}]{{laozi_2003}}",
        text,
    )
    text = re.sub(
        r"\(Zhuangzi,\s*ch\.\s*([0-9]+);\s*trans\.\s*Ziporyn,\s*2009\)",
        lambda m: f"\\cite[ch.~{m.group(1)}]{{zhuangzi_2009}}",
        text,
    )
    text = re.sub(
        r"\(Zhuangzi,\s*4th\s+century\s+BCE/2009,\s*ch\.\s*([0-9]+)\)",
        lambda m: f"\\cite[ch.~{m.group(1)}]{{zhuangzi_2009}}",
        text,
    )
    return text

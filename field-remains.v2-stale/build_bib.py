...
def collect_citations():
    """Collect citation keys from the chapter files.

    We look at both inline `(Author, Year)` tokens that did not get converted
    AND `\\cite{key}` keys already present (clean_chapters.py runs first and
    rewrites most inline cites to `\\cite{...}`). For inline matches we keep
    `(authors, year)`. For `\\cite{key}` we synthesise a `(key, year)` tuple,
    parsing the year from the trailing digits in the key.
    """
    cites = {}
    for chap in sorted(CHAPTERS_DIR.glob("ch*.tex")):
        ...
        for m in CITE_KEY_RE.finditer(text_norm):
            for k in m.group(1).split(","):
                k = k.strip()
                if not k:
                    continue
                ym = re.search(r"_(\d{4})[a-z]?$", k)
                year = ym.group(1) if ym else "0000"
                authors_synth = " and ".join(
                    p.replace("-", " ").capitalize()
                    for p in re.sub(r"_\d{4}[a-z]?$", "", k).split("_")
                )
                cites.setdefault(k, (authors_synth, year))
    return cites
...
KNOWN = {
    ...
    "mies_1986": ("@book", "Mies, M.", "Patriarchy and Accumulation on a World Scale: Women in the International Division of Labour", "Zed Books", "London", "1986"),
    "illich_1973": ("@book", "Illich, I.", "Tools for Conviviality", "Harper \\& Row", "New York", "1973"),
    "marx_1875": ("@book", "Marx, K.", "Critique of the Gotha Programme", "Progress Publishers", "Moscow", "1970"),
    "ramose_1999": ("@book", "Ramose, M. B.", "African Philosophy through Ubuntu", "Mond Books", "Harare", "1999"),
    "esteva_2014": ("@incollection", "Esteva, G.", "Commoning in the new society", "Community Development Journal 49 (suppl 1)", "", "2014"),
}
